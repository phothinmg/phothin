...
export default function responsiveTabelHook(str: string) {
  const hastTree = fromHtmlIsomorphic(str);

  const getHeaderValue = (ch: ElementContent): string => {
            ...
            .forEach((tr) => {
              (tr as Element).children.forEach((chld, i) => {
                if (chld.type === "element") {
                  chld.properties = chld.properties || {};
